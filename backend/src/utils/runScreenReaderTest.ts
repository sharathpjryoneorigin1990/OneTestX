...
test(`Accessibility test for ${screenName}`, async ({ page }, testInfo) => {
  // Define viewport sizes with type safety
  type ViewportSize = { width: number; height: number };
  type ViewportType = 'mobile' | 'tablet' | 'desktop' | 'large';
  
  const viewportSizes: Record<ViewportType, ViewportSize> = {
  ...
  const viewportType = (Object.keys(viewportSizes) as ViewportType[]).includes(viewport as ViewportType)
